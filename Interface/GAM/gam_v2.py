...
class GAM(object):
    ...
    def print_design(self, this_dict, name=None):
        ...
        if name != None:
            print(name)
        print("=============================================================================")
        print("Propulsion system definition")
        print("  Energy type = ", this_dict["power_system"]["energy_type"])
        print("  Engine type = ", this_dict["power_system"]["engine_type"])
        print("  Thruster type = ", this_dict["power_system"]["thruster_type"])
        print("  Engine count = ", "%.0f" % this_dict["n_engine"])
        print("  By Pass Ratio = ", this_dict["by_pass_ratio"])
        print("")
        print("Design mission definition")
        print("  Category = ", this_dict["airplane_type"])
        print("  Number of passenger = ", "%.0f" % this_dict["npax"])
        print("  Design range = ", "%.0f" % unit.convert_to("km", this_dict["nominal_range"]), " km")
        if this_dict["cruise_speed"] > 1:
            print("  Cruise speed = ", "%.1f" % unit.convert_to("km/h", this_dict["cruise_speed"]), " km/h")
        else:
            print("  Cruise Mach = ", "%.2f" % this_dict["cruise_speed"])
        print("  Cruise altitude = ", "%.1f" % unit.convert_to("m", this_dict["altitude_data"]["mission"]), " m")
        print("-----------------------------------------------------------------------------")
        print("Mass breakdown")
        print("  MTOW = ", "%.0f" % this_dict["mtow"], " kg")
        print("")
        print("  MZFW = ", "%.0f" % this_dict["mzfw"], " kg")
        print("  Maximum payload = ", "%.0f" % this_dict["payload_max"], " kg")
        print("  Maximum payload factor = ", "%.3f" % this_dict["max_payload_factor"])
        print("")
        print("  OWE = ", "%.0f" % this_dict["owe"], " kg")
        print("  Operator items = ", "%.0f" % this_dict["op_item"], " kg")
        print("")
        print("  MWE = ", "%.0f" % this_dict["mwe"], " kg")
        print("  Furnishing = ", "%.0f" % this_dict["furnishing"], " kg")
        print("  Standard MWE = ", "%.0f" % this_dict["std_mwe"], " kg")
        print("  Propulsion mass = ", "%.0f" % this_dict["propulsion_mass"])
        print("  Energy storage mass = ", "%.0f" % this_dict["energy_storage_mass"])
        print("  Fuel cell system mass = ", "%.0f" % this_dict["fuel_cell_system_mass"])
        print("  Basic MWE = ", "%.0f" % this_dict["basic_mwe"], " kg")
        print("  MWE factor = ", "%.3f" % this_dict["stdm_factor"])
        print("  MWE sfift = ", "%.0f" % this_dict["stdm_shift"], " kg")
        print("-----------------------------------------------------------------------------")
        print("Design mission output")
        print("  Nominal pax mass allowance = ", "%.1f" % this_dict["mpax"], " kg")
        print("  Nominal payload delta = ", "%.0f" % this_dict["delta_payload"], " kg")
        print("  Nominal payload = ", "%.0f" % this_dict["payload"], " kg")
        print("  Nominal mission time = ", "%.1f" % unit.h_s(this_dict["nominal_time"]), " h")
        print("")
        print("  Mission fuel = ", "%.0f" % this_dict["mission_fuel"], " kg")
        print("  Reserve fuel = ", "%.0f" % this_dict["reserve_fuel"], " kg")
        print("  Total fuel = ", "%.0f" % this_dict["total_fuel"], " kg")
        print("  Fuel consumption = ", "%.2f" % unit.convert_to("L", this_dict["fuel_consumption"] * unit.m_km(100)), " L/pax/100km")
        print("")
        print("  Mission energy = ", "%.3f" % unit.MWh_J(this_dict["mission_enrg"]), " MWh")
        print("  Reserve energy = ", "%.3f" % unit.MWh_J(this_dict["reserve_enrg"]), " MWh")
        print("  Total energy = ", "%.3f" % unit.MWh_J(this_dict["total_energy"]), " MWh")
        print("  Energy consumption = ", "%.2f" % unit.kWh_J(this_dict["enrg_consumption"] * unit.m_km(100)), " kWh/pax/100km")
        print("")
        print("  Wake Turbulence Category = ", this_dict["wake_turbulence_class"])
        print("-----------------------------------------------------------------------------")
        print("Factors & Efficiencies")
        print("  Max power = ", "%.3f" % unit.kW_W(this_dict["max_power"]), " MW")
        print("")
        print("  Standard mass factor", "%.4f" % this_dict["stdm_factor"])
        print("  Aerodynamic efficiency factor", "%.4f" % this_dict["aero_eff_factor"])
        print("")
        print("  Aerodynamic efficiency (L/D)", "%.2f" % this_dict["aerodynamic_efficiency"])
        print("  Propulsion system efficiency", "%.3f" % this_dict["propulsion_system_efficiency"])
        print("  Storage energy density = ", "%.0f" % unit.Wh_J(this_dict["storage_energy_density"]), " Wh/kg")
        print("  Propulsion power density = ", "%.2f" % unit.kW_W(this_dict["propulsion_power_density"]), " kW/kg")
        print("")
        print("  Structural factor (OWE/MTOW) = ", "%.2f" % this_dict["structural_factor"])
        print("  Energy efficiency factor, P.K/E = ", "%.2f" % unit.convert_to("km/kWh", this_dict["pk_o_enrg"]), " pax.km/kWh")
        print("  Mass efficiency factor, P.K/M = ", "%.2f" % unit.convert_to("km/kg", this_dict["pk_o_mass"]), " pax.km/kg")

        # print(" Maximum mass efficiency factor, P.K/M max = ", "%.2f" % unit.convert_to("km/kg", this_dict["pk_o_mass_max"]), " pax.km/kg")
        # print(" Minimum mass efficiency factor, P.K/M min = ", "%.2f" % unit.convert_to("km/kg", this_dict["pk_o_mass_min"]), " pax.km/kg")
    ...
    def print_payload_range(self, this_dict):
        ...
        print("=============================================================================")
        print("      Maximum payload = ", "%.0f" % this_dict["payload_max"], " kg")
        print(" Range of max payload = ", "%.0f" % unit.convert_to("km", this_dict["range_pl_max"]), " km")
        print("")
        print("      Nominal payload = ", "%.0f" % this_dict["payload"], " kg")
        print("        Nominal range = ", "%.0f" % unit.convert_to("km", this_dict["nominal_range"]), " km")
        print("")
        print("     Max fuel payload = ", "%.0f" % this_dict["payload_fuel_max"], " kg")
        print("       Max fuel range = ", "%.0f" % unit.convert_to("km", this_dict["range_fuel_max"]), " km")
        print("")
        print("   Zero payload range = ", "%.0f" % unit.convert_to("km", this_dict["range_no_pl"]), " km")
        print("")
